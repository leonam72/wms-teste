...
class Drawer(Base, TimestampMixin):
    ...
    license_plates: Mapped[List["LicensePlate"]] = relationship(back_populates="drawer", cascade="all, delete-orphan")
...
class UnitOfMeasure(Base, TimestampMixin):
    ...
    code: Mapped[str] = mapped_column(String, index=True) # UN, CX, PL
    description: Mapped[Optional[str]] = mapped_column(String)
    is_base: Mapped[bool] = mapped_column(Boolean, default=True)
    base_uom_id: Mapped[Optional[str]] = mapped_column(ForeignKey("units_of_measure.id"), nullable=True)
    conversion_factor: Mapped[float] = mapped_column(Float, default=1.0)

class LicensePlate(Base, TimestampMixin):
    __tablename__ = "license_plates"
    __table_args__ = (
        UniqueConstraint("code", name="uq_lpn_code"),
    )
    code: Mapped[str] = mapped_column(String, unique=True, index=True) # LPN0001
    lpn_type: Mapped[str] = mapped_column(String, default="pallet") # pallet, box, container
    drawer_id: Mapped[Optional[str]] = mapped_column(ForeignKey("drawers.id"), index=True, nullable=True)
    status: Mapped[str] = mapped_column(String, default="active") # active, closed, shipping
    
    drawer: Mapped[Optional["Drawer"]] = relationship(back_populates="license_plates")
    stock_items: Mapped[List["StockItem"]] = relationship(back_populates="license_plate", cascade="all, delete-orphan")
...
class Product(Base, TimestampMixin):
    ...
    unit: Mapped[Optional[str]] = mapped_column(String) # un, cx, kg (legacy string)
    uom_id: Mapped[Optional[str]] = mapped_column(ForeignKey("units_of_measure.id"), nullable=True)
    ...
    unit_of_measure: Mapped[Optional["UnitOfMeasure"]] = relationship()
...
class StockItem(Base, TimestampMixin):
    ...
    drawer_id: Mapped[Optional[str]] = mapped_column(ForeignKey("drawers.id"), nullable=True, index=True)
    license_plate_id: Mapped[Optional[str]] = mapped_column(ForeignKey("license_plates.id"), nullable=True, index=True)
    status: Mapped[str] = mapped_column(String, default="available", index=True) # available, blocked, reserved, quarantine
    ...
    drawer: Mapped[Optional["Drawer"]] = relationship(back_populates="stock_items")
    license_plate: Mapped[Optional["LicensePlate"]] = relationship(back_populates="stock_items")
...
class Task(Base, TimestampMixin):
    ...
    task_type: Mapped[str] = mapped_column(String, index=True) # putaway, picking, replenishment, internal_move
    status: Mapped[str] = mapped_column(String, default="pending", index=True) # pending, assigned, in_progress, completed, cancelled
    ...
    license_plate_id: Mapped[Optional[str]] = mapped_column(ForeignKey("license_plates.id"), nullable=True)
    product_id: Mapped[Optional[str]] = mapped_column(ForeignKey("products.id"), nullable=True)
    quantity: Mapped[int] = mapped_column(Integer, default=0)
    
    # Rastreamento de Movimentação
    from_drawer_id: Mapped[Optional[str]] = mapped_column(ForeignKey("drawers.id"), nullable=True)
    to_drawer_id: Mapped[Optional[str]] = mapped_column(ForeignKey("drawers.id"), nullable=True)
    ...
    # Relationships
    license_plate: Mapped[Optional["LicensePlate"]] = relationship()
    product: Mapped[Optional["Product"]] = relationship()
    from_drawer: Mapped[Optional["Drawer"]] = relationship(foreign_keys=[from_drawer_id])
    to_drawer: Mapped[Optional["Drawer"]] = relationship(foreign_keys=[to_drawer_id])
