...
async def approve_receiving_session(
    ...
    for item in itens_ok:
        ...
        drawer_key = _find_destination_drawer(
            next_state, depot_id, incoming_kg,
            product_code=stock_entry.get("code"), blocked_only=False
        )
        if not drawer_key:
            raise HTTPException(
                status_code=409,
        ...
        next_state["history"].insert(0, {
            ...
            "detail": f"NF {session.numero_nf}: {stock_entry.get('code')} → {drawer_key}",
    ...
    for item in itens_avariados:
        ...
        incoming_kg = _safe_float(stock_entry.get("kgTotal"))
        blocked_target = _find_blocked_destination(next_state, incoming_kg, stock_entry.get("code"))
        if not blocked_target:
            raise HTTPException(status_code=409, detail="Nenhuma área bloqueada disponível para itens avariados.")
        target_depot_id, target_drawer_key = blocked_target
        ...
        next_state["history"].insert(0, {
            ...
            "detail": f"NF {session.numero_nf}: {stock_entry.get('code')} → {target_drawer_key} (BLOQUEADO)",
            "icon": "🛡", "user": current_user.username,
